...
export default class ProductDetails {
    constructor($scope, context, productAttributesData = {}) {
        ...
        const $productOptionsElement = $('[data-product-option-change2]', $form);
        ...
        if (_.isEmpty(productAttributesData)) {
    ...
    addProductToCart(event, form) {
        ...
        utils.api.cart.itemAdd(new FormData(form), (err, response) => {
            ...
            if ($(window).width() < 1200 && $('.module-wrapper.productList').length == 0) {
                ...
                $addToCartBtn.parents('.card-figure').removeClass('has-popup');
                $addToCartBtn.parents('.card').removeClass('has-popup');
                $modal_bg.remove();
